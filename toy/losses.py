...
""" oringal license TensorFlow Teams """

""" Modified code from tensorflow.org
and added custom codes by John Park 
...
def get_class2distance(DistanceFilename):	
	distance = dict()### 'Class1 <=> Class2' => distance
	with open(DistanceFilename, mode = 'rt') as file:
		for k, line in enumerate(file):
			if k > 0:
				columns = line.rstrip().split('\t')
				distance[f"{columns[0]} <=> {columns[1]}"] = float(columns[2])
	class2distance = tf.lookup.StaticHashTable( ### 'Class1 <=> Class2' => distance
...
def get_label2class(ls_label_int, ls_class_str):
    label2class = tf.lookup.StaticHashTable( ### taxonID => Genus
        tf.lookup.KeyValueTensorInitializer(
            tf.constant(list(ls_label_int), dtype = tf.int32), 
            tf.constant(list(ls_class_str), dtype = tf.string)
            ),
            default_value = 'UNKNOWN MAPPING'
        )

    return label2class


def get_map_label_str_to_int(df, keycol, sort_alphabetically=True):
    if sort_alphabetically:
      S_scif = sorted(set(df[keycol]))
    else:
      S_scif = set(df[keycol])
    return dict(zip(S_scif, range(len(S_scif))))
    

def get_label_to_class_from_path(df_PATH, keycol, valcol, convert_key_to_int =True,
    ...
    if convert_key_to_int:
      mapping_scif2int = get_map_label_str_to_int(df, keycol, 
    ...
    return get_label2class(ls_label, ls_class)


def get_key_case_int(TrueClass, PredClass):
...
class WeightedCrossentropy(LossFunctionWrapper):
  """ Patristic Distance Crossentropy : Cross entropy loss that sample weights are distance between
